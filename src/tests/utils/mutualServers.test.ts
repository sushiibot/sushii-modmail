...
describe("getMutualServers", () => {
  it("returns guilds where user exists", async () => {
    const guild1 = { id: "1", name: "G1", members: { fetch: mock().mockResolvedValue({}) } };
    const guild2 = { id: "2", name: "G2", members: { fetch: mock().mockResolvedValue({}) } };
    const client = {
  ...
  it("skips guilds where member not found", async () => {
    const error = new DiscordAPIError({ message: "" }, RESTJSONErrorCodes.UnknownMember, 404, "GET", "", { body: undefined, files: undefined });
    const guild1 = { id: "1", name: "G1", members: { fetch: mock().mockRejectedValue(error) } };
    const guild2 = { id: "2", name: "G2", members: { fetch: mock().mockResolvedValue({}) } };
    const client = { guilds: { cache: { values: () => [guild1, guild2] } } } as unknown as Client;
  ...
  it("throws unexpected errors", async () => {
    ...
    const guild = { id: "1", name: "G1", members: { fetch: mock().mockRejectedValue(err) } };
    const client = { guilds: { cache: { values: () => [guild] } } } as unknown as Client;
