...
INSERT INTO `__new_messages`("thread_id", "message_id", "author_id", "is_staff", "staff_relayed_message_id", "user_dm_message_id", "content", "forwarded", "is_anonymous", "is_plain_text", "is_snippet") SELECT "thread_id", "message_id", "author_id", "is_staff", "staff_relayed_message_id", "user_dm_message_id", "content", "forwarded", "is_anonymous", "is_plain_text", "is_snippet" FROM `messages`;--> statement-breakpoint
...
CREATE TABLE `__new_threads` (
	...
	`title` text,
	`created_at` integer DEFAULT '"2025-05-04T22:09:03.070Z"' NOT NULL,
	`closed_at` integer,
	`closed_by` text,
