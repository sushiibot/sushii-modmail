CREATE TABLE `bot_roster_seed_state` (
	`id` integer PRIMARY KEY NOT NULL,
	`seeded_at` integer DEFAULT (strftime('%s','now')) NOT NULL,
	CONSTRAINT "single_row_check" CHECK("bot_roster_seed_state"."id" = 1)
);
...
CREATE TABLE `bots` (
	...
	CONSTRAINT "mail_guild_id_check" CHECK("bots"."mail_guild_id" NOT GLOB '*[^0-9]*'),
	CONSTRAINT "application_id_check" CHECK("bots"."application_id" NOT GLOB '*[^0-9]*')
);
--> statement-breakpoint
...
CREATE TABLE `__new_threads` (
	...
	`title` text,
	`created_at` integer DEFAULT '"2026-08-09T17:14:08.025Z"' NOT NULL,
	`closed_at` integer,
	`closed_by` text,
