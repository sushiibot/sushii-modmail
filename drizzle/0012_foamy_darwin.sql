CREATE TABLE `additional_message_ids` (
	`main_message_id` text PRIMARY KEY NOT NULL,
	`additional_message_id` text NOT NULL,
	FOREIGN KEY (`main_message_id`) REFERENCES `messages`(`message_id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX `additional_message_ids_additionalMessageId_unique` ON `additional_message_ids` (`additional_message_id`);--> statement-breakpoint
...
CREATE TABLE `__new_threads` (
	...
	`title` text,
	`created_at` integer DEFAULT '"2025-05-27T04:55:14.257Z"' NOT NULL,
	`closed_at` integer,
	`closed_by` text,
